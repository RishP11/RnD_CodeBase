...
def svdMIMOencoder(input_stream, csi, E_b=1):
    ...
    for i in range(num_pckts):
        U, S, Vh = np.linalg.svd(csi[i])
        tempSeq = [input_stream[idx], input_stream[idx+1]]
        txOut = Vh @ (BPSK_mapper(tempSeq), E_b)
        normalized_txOut = np.array(E_b) * txOut / np.linalg.norm(txOut)
        antenna_01.append(normalized_txOut[0])
        antenna_02.append(normalized_txOut[1])
    
    return np.array([antenna_01, antenna_02])
