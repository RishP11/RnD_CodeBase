# Deep Learning Applications in Digital Communications<br>
# By Rishabh Pomaje
#-----------------------------------------------------------------------------------
# >>> Abstract  
...
# In this script we will be trying to learn (15, 11) Channel code 
#-----------------------------------------------------------------------------------
...
training_data_size = 10 ** 5 # in number of blocks of k bits
...
axes.semilogy(SNR_dB, BLER_coded_hard, label="Hamming (15, 11) Hard", c="blue", ls="-.")
axes.semilogy(SNR_dB, BLER_learned, label="Learned System", c='red', marker='o', ls=" ")
axes.semilogy(SNR_dB, BLER_coded_MLD, label="Hamming (15, 11) MLD", c="blue", ls="--")
axes.set_xlabel(r'$E_b / N_0[dB]$')
axes.set_ylabel(r'$P_{BLE}$')
axes.set_xlim(-4, 8)
axes.set_ylim(10**-5, 10**0)
axes.set_title(r'BPSK with SNR$_{train} = 10.5dB$')
axes.legend()
axes.grid()
#-----------------------------------------------------------------------------------
fig.savefig(fname='plots/deepComms_15_11_small_SNR_10.5dB.svg', transparent=True)

print(BLER_learned) # Learned schemes block error rate

with open("results/results_deepComms_15_11_small_SNR_10.5dB.txt", mode='w') as file_id :
